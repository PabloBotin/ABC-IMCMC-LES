...
# Plot log pdf of production rate
    def LES_logpdf_production(self, label):
        ...
        plt.plot(production_pdf[0],y_true[3], label='DNS', c= 'k', lw= 1.5)
        plt.plot(production_pdf[0], production_logpdf, label= label, ls='--', c= 'r', lw= 2)     
        plt.ylim([-5,5])
        plt.show()
        # plt.hist(self.production_data, bins='auto', range=[-1.2, 0.01], density=1)
...
ABC_static_test = plots_from_LES (folder1, 'case D')
...
# Log pdf Production
ABC_static_test.LES_logpdf_production('4 param')
# ABC_static_test_B.LES_logpdf_production('4 param')


# Log pdf Sigma [11 12 13]
ABC_static_test.LES_logpdf_sigma('4 param')
