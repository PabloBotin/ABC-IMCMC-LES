...
import sys
sys.path.insert(0, "/Users/pablo/github/teslapy")
...
class plots_from_LES:
    ...
    # def find_file_Production (self):
    #     self.file_production = os.path.join(self.folder, f'test-Production_001.rst')

    # def find_file_Sigma (self):
    #     self.file_sigma = os.path.join(self.folder, f'test-tau11_001.rst')

    # Finds the 3 files of sogma corresponding to 11, 12, 13. 
    # def find_3files_Sigma (self):
...
# Plot log pdf of production rate
    def plot_logpdf_production(self):
        production_pdf= pdf_from_array_with_x(self.production_data, 100, [-5, 5])
        production_logpdf = take_safe_log(production_pdf[1])
        plt.plot(production_pdf[0], production_logpdf)
        plt.ylim([-9,2.5])
        # plt.hist(self.production_data, bins='auto', range=[-1.2, 0.01], density=1)
        plt.title("log pdf of the production rate")
        plt.show()

# Plot pdf of logarithm of sigma. Esto est'a ma'
    def plot_logpdf_sigma(self):
        ...
        plt.plot(sigma_pdf[0], sigma_logpdf)
        plt.ylim([-5,4])
...
# # Plot pdf of logarithm of sigma. Esto est'a ma'
#     def plot_logpdf_sigma(self):
#         sigma_pdf= pdf_from_array_with_x(self.sigma_data, 100, [-0.3, 0.3])
#         sigma_logpdf = take_safe_log(sigma_pdf[1])
#         plt.plot(sigma_pdf[0], sigma_logpdf)
#         plt.ylim([-5,4])
#         plt.title("pdf of the deviatoric part of the stress tensor")
#         plt.show()
        ...
        # plot(sigma_ln_pdf, bins='auto', range=[-7.9, -2], density=1)
        # plt.plot(sigma_ln_pdf)
        # plt.title("pdf of the logarithm of the deviatoric part of the stress tensor")
        # plt.savefig("logplot.png")
        # plt.show()
        # lw = 1
        # y_true = np.loadtxt(os.path.join(self.folder, 'sum_stat_true'))
        # x_true = np.loadtxt(os.path.join(self.folder, 'sum_stat_bins'))
        # x = x_true[0]

        # probability = np.loadtxt(os.path.join(folder, 'probability_from_posterior_'+str(4)))
        # if len(probability.shape) > 1:
        #     print('old probability format')
        #     probability = probability[:, 0]
        # probability /= probability[0]

        # fig, axarr = plt.subplots(nrows=3, ncols=4, sharey=True, figsize=(fig_width, fig_height))
        # for i, pdf_ij in enumerate(sigma_ln_pdf[:3]):
        #     for j, pdf in enumerate(pdf_ij[1:]):
        #         axarr[i, j].plot(x, pdf, 'm-', linewidth=probability[j]*lw, alpha=probability[j])
...
# RUN BABY RUN 
folder = '/Users/pablo/Documents/Pablo_Stats/stats_from_LES/datafiles'
ABC_static_test = plots_from_LES (folder)
# ABC_static_test.plot_logpdf_production()
# ABC_static_test.plot_logpdf_sigma()
# ABC_static_test.plot_sigma_ij()
# ABC_static_test.plot_production()




# [np.min(tau_), np.max(tau_)]
