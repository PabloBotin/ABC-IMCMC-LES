...
Description:
------------
Version F from Table 1. Data from production/Sigma, 4 param. C = [-0.032, -0.014, -0.2, -0.2]
------------
Pope75 static coefficient forward testing

Notes:
------
run `mpiexec -n 4 python ABC_static_test_F.py -h` for help
...
def ABC_static_test(C=None, run=1, pp=None, sp=None, adir=None, odir=None):
    ...
    if C is None:
        C = np.array([-0.032, -0.014, -0.2, -0.2])
