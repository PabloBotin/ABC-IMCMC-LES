...
Description:
------------
Version D from Table 1. Data from production, 4 param. C = [-0.037, -0.09, -0.13, 0.22]
128 grid
------------
...
Notes:
------
run `mpiexec -n 4 python ABC_static_test_D_128.py -h` for help
...
def ABC_static_test(C=None, run=1, pp=None, sp=None, adir=None, odir=None):
    ...
    if C is None:
        C = np.array([-0.037, -0.09, -0.13, 0.22])
    ...
    while t_sim < pp.tlimit+1.e-8:
        ...
        if t_test >= t_rst:
            ...
            for i in range(3):
                writer.write_scalar('%s-Velocity%d_%3.3d.rst' % (pp.pid, i+1, irst), U[i], np.float128)
                for j in range(3):
                    writer.write_scalar('%s-tau%d%d_%3.3d.rst' % (pp.pid, i+1, j+1 , irst), solver.tau_ij[i, j], np.float128)   
                    production += solver.tau_ij[i, j]*solver.S[i, j]
            writer.write_scalar('%s-Production_%3.3d.rst' % (pp.pid, irst), production, np.float128)                    
            t_rst += dt_rst
            irst += 1
...
time_group.add_argument('-t', '--tlimit', type=float, default=0.8,
