...
# Plot log pdf of production rate
    def LES_logpdf_production(self):
        ...
        plt.plot(production_pdf[0],y_true[3], label='DNS')
        plt.plot(production_pdf[0], production_logpdf, label='LES - 4 parameters')
        plt.legend()      
        plt.ylim([-9,5])
        # plt.hist(self.production_data, bins='auto', range=[-1.2, 0.01], density=1)
        plt.title("log pdf of LES and DNS production rate C = [-0.069, 0.07, 0.0056, 0]")
        plt.show()
...
# Plot log of pdf of sigma. 
    def LES_logpdf_sigma(self):
        ...
        for i in range (3):
            ...
            axarr[i].plot(sigma_pdf[i][0], sigma_logpdf[i], label='LES - 4 parameters')
            axarr[i].plot(sigma_pdf[i][0], y_true[i], label='DNS')

        plt.style.use('fivethirtyeight')
        plt.legend(["DNS","LES - 4 parameters"])  
        plt.suptitle('log pdf of LES and DNS production rate C = [-0.069, 0.07, 0.0056, 0]')  
        # Not able to set the title at the center. Try with plt.suptitle()
        plt.ylim([-5,5]) 
        plt.show()
