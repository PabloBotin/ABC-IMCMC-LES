...
from spectrallesPablo import Config, SpectralLES  # only works when run.py and spectralles.py are in the same directory!
...
def main_abc_program():
    ...
    # from a h5py file and Fourier transforms it into u. 
    # so we have in memory the 3D velocities, from there we can compute statistics 
    # But first, I need to specify the file where the 3D velocities are stored:
    sim.initialize_from_file(fh)
    print (sim.u)
    # Compute the summary statistics from velocities
    # Only Pi (Production Rate) currently
