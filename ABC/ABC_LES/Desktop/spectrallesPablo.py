...
class Config:
    ...
    # Initial E(k) slope if init_cond == 'file'
    kExp_init: float = 1/3
...
class SpectralLES(MPI_Debugging):
    ...
    def finalize(self, t_sim=None):
        ...
        # output velocities without updating self.t_stat
        # self.save_velocities_to_file(config.t_init)
    ...
    # def save_velocities_to_file(self, t_sim):
    #     iK = self.iK
    #     u_hat = self.u_hat
    #     work = self.work[0]
    ...
    #     # Save the velocities to the Velocities File
    #     # u = 
    #     # v =
    #     # w = 
    #     # if self.comm.rank == 0:
    ...
    def save_histogram_to_file(self, fh, data, key):
        ...
        temp, edges = np.histogram(data, bins=bins, range=xrange)
